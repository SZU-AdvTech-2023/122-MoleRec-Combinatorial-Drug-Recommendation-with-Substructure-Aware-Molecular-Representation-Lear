...
def Train(
    ...
    for epoch in range(EPOCH):
        ...
        for step, input_seq in enumerate(data_train):
            # 每位患者的每次就诊都可以作为一次预测，一位患者的第n次就诊，用前n-1次就诊记录来预测第n次的结果，并计算损失
            # 这样应该可以充分利用数据
            for adm_idx, adm in enumerate(input_seq):
                # 创建一个数组长度voc_size[2]=131，代表131种药物
                ...
                # todo:删掉多标签损失
                # loss_multi = (sigmoid_res + multi_target) * 0.5
                ...
                if current_ddi_rate <= target_ddi:
                    ...
                    loss = beta * (0.95 * loss_bce + 0.05 * loss_multi) \
                        + (1 - beta) * loss_ddi
                # todo： 消融实验线性优化策略：linear adjusting
                # loss = a(0.95 * loss_bce + 0.05 * loss_multi)
                # todo：消融试验：不使用多标签损失
                # if current_ddi_rate <= target_ddi:
                #     loss = loss_bce
                # else:
